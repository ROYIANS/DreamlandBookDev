window.addEventListener('load', () => {
  ...
  const openSearch = () => {
    ...
    dlf.animateIn($searchMask, 'to_show 0.5s')
    dlf.animateIn(document.querySelector('#local-search .search-dialog'), 'titleScale 0.5s')
  ...
  const closeSearch = () => {
    ...
    bodyStyle.width = ''
    bodyStyle.overflow = ''
    dlf.animateOut(document.querySelector('#local-search .search-dialog'), 'search_close .5s')
    dlf.animateOut($searchMask, 'to_hide 0.5s')
  }
  ...
  window.addEventListener('pjax:complete', () => {
    !dlf.isHidden($searchMask) && closeSearch()
    searchClickFn()
  })
})
